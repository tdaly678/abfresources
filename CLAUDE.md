...
**2026-09-23 - Attendance instructions PDF replaced (Sept 2026 revision).** `ABF-Attendance-Instructions.pdf` at the repo root was swapped for Tom's updated version (source: `~/Downloads/ABF_Attendance_Instructions.pdf`, dated Sept 15). It grows from 6 pages to 7: the new page 4, **"Adding Visitors and New Attenders,"** covers the `...` menu next to Take Attendance, the Group Leader Toolbox, and the pre-filled add-person form. All other text is unchanged; the screenshots are larger and cleaner (621 KB to 2.4 MB). The home page flags the change in two places, both new and both small: a `.hc-updated` line under the attendance carousel slide's buttons, and a `.hnc-updated` line on the Take Attendance Quick Access card. Both read "Updated September 2026." Retire those two notes once leaders have had a few weeks with the new version.
